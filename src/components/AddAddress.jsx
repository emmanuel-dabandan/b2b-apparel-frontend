...
export default function AddAddress({ onSave, onCancel }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await onSave({ name, address, city, state, zip, is_default: isDefault });
    } catch (err) {
      ...
      <div className="col-lg-8 mt-4">
        ...
        <div className="card shadow-sm border-0 rounded-4 overflow-hidden bg-body border">
          <div className="card-header bg-dark text-white fw-bold py-3">
            <i className="bi bi-geo-alt-fill me-2"></i>Add New Address
          </div>
          <div className="card-body p-4 p-md-5">
            ...
            <form onSubmit={handleSubmit}>
              <div className="mb-4">
                <label className="form-label fw-bold small text-muted">Location Name (e.g., Home, Office)</label>
              ...
              <div className="mb-4 form-check">
                <input type="checkbox" className="form-check-input shadow-none" id="defaultAddress" checked={isDefault} onChange={(e) => setIsDefault(e.target.checked)} />
                <label className="form-check-label small fw-semibold text-muted" htmlFor="defaultAddress">Set as default shipping address</label>
              </div>
              ...
              <div className="d-flex justify-content-end gap-3 mt-5">
                ...
                <button type="submit" className="btn btn-primary fw-semibold px-5 rounded-pill d-flex align-items-center gap-2" disabled={isLoading}>
                  {isLoading ? <span className="spinner-border spinner-border-sm"></span> : 'Save Address'}
                </button>
              </div>
            </form>
          </div>
